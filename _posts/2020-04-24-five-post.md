...
## [Computer Vision]
    ...
    &diams; Recognition : Object가 어떤 것인지 구분
    &diams; Object Detection : Recognition보다 더 작은 범위로써 Object의 존재 유무만 판단

## [Object Detection]

    영상이 들어오면 한 부분에 물체가 있다는걸 인식(Object Recognition)하고, 그 물체가 무엇인지(Object Classification)하고 정확한 위치를 찍어줌(Object Localization)
    &diams; Two-shot-detection (초기 버전) : Two-shot이란 2단계를 걸쳐서 검출

    &diams; Two-shot-detection (업그레이드 버전) : Regional proposal과 Classification이 순차적으로 이루어짐
